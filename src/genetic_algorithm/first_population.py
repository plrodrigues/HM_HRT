...
def get_possible_collaborative_modes(
    instance: Instance, working_space: int = None
) -> list[str]:
    if not working_space:
        working_space = instance.df_workingspace_id.WorkingSpace.min()
...
def get_possible_modes(
    ...
    collaborative_modes = get_possible_collaborative_modes(
        instance, working_space
    )
...
def get_permutations_for_order_of_tasks(
    ...
    permutations_of_oders = [
        list(itertools.permutations(order_section))
...
def get_first_population(
    possible_modes: list[str], number_tasks: int
) -> list[Chromosome]:
    # Chromosomes
    chromosomes = []
    # Order of tasks
    ...
    for n_order in order_permutations:
        for m_mode in possible_modes:
            chromosome_x = Chromosome(
                mode=[m_mode for _ in range(number_tasks)], order=n_order
            )
