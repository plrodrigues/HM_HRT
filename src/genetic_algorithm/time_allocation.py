...
from src.genetic_algorithm import first_population, replication
...
def get_possible_modes(instance: Instance) -> list[str]:
    ...
    for ws in instance.df_workingspace_id.WorkingSpace.unique():
        ...
        collab_modes = first_population.get_possible_collaborative_modes(
            instance, ws
        )
...
def get_time_from_single_resource(
    instance: Instance, mode: str, task: int
) -> int:
...
def find_end_time_of_task_in_allocated_modes(
    allocated_modes: dict, task: int
) -> int:
...
def get_all_time_allocations(
    ...
    for task in tasks:
        ...
        if replication.is_int(mode_of_class):
            time_of_task_in_mode = get_time_from_single_resource(
                instance, mode_of_class, task
            )
        else:
            resource_1, resource_2 = replication.split_collaboration_into_ints(
                mode_of_class
            )
        ...
        if task not in times_allocated[mode_of_class]:
            ...
            if len(predecessors) > 0:
                for pred in predecessors:
                    previous_predecessor_end_time = (
                        find_end_time_of_task_in_allocated_modes(
                            times_allocated, pred
                        )
                    )
                    if previous_predecessor_end_time:
                        if (previous_predecessor_end_time > predecessors_allocated_at):
                            predecessors_allocated_at = (
                                previous_predecessor_end_time
                            )

            # Verify if mode is allocated alone or in team already
            mode_available_time = find_the_next_time_slot_of_mode(
                times_allocated, mode_of_class
            )
            this_start_time = (
                max(mode_available_time, predecessors_allocated_at) + 1
            )
